...
def getoneByte(
        hex_value) -> str:  # return value 1. onebyte value, 2. rest of value
    return str(hex_value[:2]), str(hex_value[2:])


def calculate_hex(
        hex_value: str):  # get a byte value and translate to binary -> return
    binary_value = bin(0)
    if (hex_value != ''):
...
def ASN1_parser(value: str) -> dict:  # make data to be tag+length+value
    ...
    object_byte_binary = calculate_hex(
        object_byte)  # get binary of a byte ex: a1 -> 10100001
